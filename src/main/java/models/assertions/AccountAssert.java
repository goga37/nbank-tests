...
public class AccountAssert extends AbstractAssert<AccountAssert, AccountResponse> {
    ...
    // с нужным типом, суммой и связанным счётом.
    // "Ровно одна" — потому что дубль транзакции тоже был бы багом.
    public AccountAssert hasTransaction(String type, double amount, long relatedAccountId) {
        isNotNull();

        List<Transaction> matching = actual.getTransactions().stream()
                .filter(tx -> type.equals(tx.getType()))
                .filter(tx -> Math.abs(tx.getAmount() - amount) <= AMOUNT_TOLERANCE)
                .filter(tx -> tx.getRelatedAccountId() == relatedAccountId)
                .toList();

        if (matching.isEmpty()) {
            failWithMessage(
                    "Expected transaction [type=%s, amount=%s, relatedAccountId=%d] not found.\nActual transactions: %s",
                    type, amount, relatedAccountId, actual.getTransactions()
            );
        } else if (matching.size() > 1) {
            failWithMessage(
                    "Expected exactly 1 transaction [type=%s, amount=%s, relatedAccountId=%d] but found <%d>.\nMatches: %s",
                    type, amount, relatedAccountId, matching.size(), matching
            );
        }
